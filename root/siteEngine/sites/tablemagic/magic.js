var tab = "csv", header_alignment = [], array_storage = "", form_rows = 0, form_cols = 0, prettify_md = true,
    debug = false, global_form_cols = 0, delete_mode = false;
...
function changeTab(newTab) {
    ...
    if (tab !== newTab) {
        ...
        if (tab === "md") {
            array = md2array(input);
        }
        if (tab === "csv") {
            array = csv2array(input);
        }
        if (tab === "html") {
            array = html2array(input);
        }
        if (tab === "sql") {
            array = sql2array(input);
        }
        if (tab === "form") {
            array = form2array();
        }
...
function csv2array(csv) {
    ...
    for (var c = 0; c < csv.length; c++) {
        ...
        if (save) {
            ...
            if (((char === ",") && ((mode === 0) || (mode === 1))) || (char === "\t") && ((mode === 0) || (mode === 2))) {
                col++;
            }
            if (char === "\n") {
                col = 0;
...
function md2array(md) {
    ...
    if (array[1]) {
        ...
        for (var h = 0; h < headers.length; h++) {
            ...
            if ((start) && (!end)) {
                header_alignment[h] = "l";
            }
            if ((start) && (end)) {
                header_alignment[h] = "c";
            }
            if ((!start) && (end)) {
                header_alignment[h] = "r";
            }
...
function array2sql(array) {
    ...
    for (var r = 0; r < array.length; r++) {
        ...
        if (r == 0) {
            md += array2sqlDashes(cell_sizes, row.length) + "\n";
        }
        ...
        if (r == 0) {
            ...
            for (var c = 0; c < row.length; c++) {
                ...
                if (prettify_md) {
                    ...
                    if (header_alignment[c] === "c") {
                        spaces = spaces - 2;
                    }
                    if (header_alignment[c] === "r") {
                        spaces = spaces - 1;
                    }
